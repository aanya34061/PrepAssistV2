...
export default function UserQuestionBank() {
  ...
  const loadLibrary = async () => {
    setIsLoading(true);
    const data = await fetchQuestions(100); 
    setQuestions(data);
    setIsLoading(false);
  };
  ...
  if (questions.length === 0) {
     return (
        <div className="min-h-[80vh] flex flex-col items-center justify-center p-6 text-center bg-white">
           ...
           <p className="text-slate-500 font-medium max-w-sm">No active questions have been injected into the global arrays by administrators yet.</p>
        </div>
     );
  }
